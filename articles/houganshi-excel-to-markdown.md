...
ここからが本命です。従来ツールは「セル構造」をそのままMarkdownに写しますが、LLMは「意味構造」を読み取って再構成します。方眼紙Excelの「視覚レイアウト→論理構造」変換は、現時点ではLLMが最も現実的な選択肢です。
...
PILでシートをPNG化してgpt-4oに渡した結果です。**図・グラフを含むシートでどう変わるか**がポイントです。
...
- **図・グラフ**：テキストモード同様 `[図: 配置図面]` として出力。画像化してもグラフの内容は記述されない
- **ガントチャート**：列ヘッダー（月名）と工程名は正確に読み取れるが、**セルの色（青/橙）はMarkdownに変換できない**。「どの月に工程があるか」という情報が失われる
- **OCR精度**：画像化による文字認識誤りが発生（「構造用合板」が「断熱材」に誤認識など）
:::

**結論：ビジョンモードはテキストモードより優れているわけではない**

| 観点 | テキストモード | ビジョンモード |
|------|--------------|----------------|
| 文字精度 | ✅ 高い（セルの値を直接取得） | △ OCR誤りあり |
| 図の変換 | `[図: ...]` プレースホルダ | `[図: ...]` プレースホルダ（変わらず） |
| ガントチャート | ✅ セルのテキスト値を取得 | ❌ 色情報が失われる |
| 処理時間 | 〜20秒 | 〜29秒（PNG生成込み） |

ビジョンモードは「画像しかない場合」に有効ですが、Excelセルが直接読めるならテキストモードの方が精度・速度ともに有利です。
...
| ツール | 方眼紙対応 | 50列問題 | 図・グラフ | ガントチャート | 処理速度 | 導入難度 | コスト |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| openpyxl | △ | ❌ 残る | ❌ | ❌ | ⚡ 速い | 低 | 無料 |
...
| GitHub Models (text) | ✅ | ✅ **解決** | △ プレースホルダ | ✅ セルテキスト取得 | ○ 〜20秒 | 中 | 無料枠あり |
| GitHub Models (vision) | ✅ | ✅ **解決** | △ プレースホルダ | ❌ **色情報が失われる** | ○ 〜29秒 | 中 | 無料枠あり |
| GitHub Copilot SDK (gpt-4.1) | ✅ | ✅ **解決** | △ プレースホルダ | ✅ セルテキスト取得 | 🐢 〜113秒 | 中 | Copilotサブスク必要 |
| GitHub Copilot SDK (claude-sonnet-4.6) | ✅ | ✅ **解決** | △ プレースホルダ | ✅ セルテキスト取得 | 🐢 〜56秒 | 中 | Copilotサブスク必要 |
...
方眼紙Excelは「視覚レイアウト」で意味を表現する形式です。Markdownは「論理構造」で表現します。この2つはそもそも表現モデルが違うため、セル構造を機械的に変換しても意味のあるMarkdownにはなりません。現時点では、LLMが最も現実的な選択肢です。
...
| 画像・図面も含めて変換したい | **テキストモード推奨**（ビジョンモードはOCR誤りあり・ガントチャートの色情報も失われる） |
